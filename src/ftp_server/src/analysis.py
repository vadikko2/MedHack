...
def return_data(path_name, d):
    ...
    for l in _list:
        if  l == path_name:
            ...
            os.chdir('/home/vadim/hackatones/medhack/src/ftp_server/data/sample/')
            file_name = os.listdir()
            time_str = file_name[0]
            time_now = datetime.strptime(time_str[:-4] ,"%d_%m_%Y_%H_%M")
        i+=1
        return name, time_now, [dist_x[0][0], dist_y[0][0], dist_z[0][0]], y_svm, marks
